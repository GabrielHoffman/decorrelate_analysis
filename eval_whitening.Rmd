...
```{r all, fig.width=12, fig.height=4, cache=FALSE}
lapply(c("EUR", "AFR", "ASN"), function(pop){
...
```{r all,mean, fig.width=12, fig.height=12, cache=FALSE}
...
df %>%
	...
	ggplot(aes(method, mean, fill=method)) + 
		...
		geom_hline(yintercept=100, color="grey70", linetype="dashed") + 
		theme_classic()  +
		theme(aspect.ratio=1, 
			plot.title = element_text(hjust = 0.5),
			legend.position="none") +

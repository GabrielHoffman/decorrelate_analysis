...
df %>%
	...
	# mutate(rel.err = abs(rel.err)) %>%
	# mutate(rel.err = rMSE / rMSE_baseline) %>%
	mutate(rel.err = rMSE_baseline - rMSE ) %>%
...
df %>%
	...
	filter(category == '(300,500]', pop == "EUR")
...
df2 = df %>%
	...
	mutate(meanValue = mean, 
				mean = pmin(mean, 55), 
		  	se = ifelse(mean<55,se,0)) 

df2 %>%
	ggplot(aes(method, mean, fill=method)) + 
		geom_bar(stat="identity") +
		geom_errorbar(aes(ymin = mean - 1.96*se, ymax=mean + 1.96*se), width=0) +
		geom_text(data=filter(df2, mean == 55), aes(method, 47, label=round(meanValue, digits=1))) +
		theme_classic()  +
		theme(aspect.ratio=1, 
			plot.title = element_text(hjust = 0.5),
			legend.position="none") +
		...
		theme(aspect.ratio=1) +
		ylab("Relative reduction in rMSE (%)") +
		coord_flip(expand=FALSE, ylim=c(0, 55) ) + 
		scale_fill_manual(name="Method",values = cols)
```

...
ggplot(df, aes(p, rmse, color=Method)) + 
  geom_point(alpha=.2) + 
  theme_classic() + 
  theme(aspect.ratio=1, plot.title = element_text(hjust=0.5)) + 
  xlab("Number of features") + 
...
ymax = 1.02*max(df$rmse[df$Method == 'lambda = 0.1'])
ymin = 0.98*min(df$rmse)
...
```{r block_correlation, cache=FALSE, fig.height=4, fig.width=12}ggplot(df, aes(p, rmse, color=Method)) + 
  geom_point(alpha=.2) + 
  theme_classic() + 
  theme(aspect.ratio=1, plot.title = element_text(hjust=0.5)) + 
  xlab("Number of features") + 
...
ymax = 1.02*max(100*df2$rel.err[df2$Method == 'lambda = 0.1'])
ymin = .98*min(100*df2$rel.err[df2$Method == 'Oracle'])

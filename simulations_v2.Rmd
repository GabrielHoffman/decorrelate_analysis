...
ymax = df %>%
  filter(((Method == 'lambda = 0') & (p < 0.49*n))) %>%
...
fig = df2 %>%
  ...
  coord_fixed(xlim = c(0, NA), ylim=c(0, ymax), expand = FALSE) +
  scale_color_manual(name="Method",values = colMethods) + 
  geom_smooth(method='loess', se=FALSE, span=.35, size=1.5) +
...
plot_grid(fig + theme(legend.position="none"), 
        fig + coord_cartesian(ylim = c(ymin, ymax), xlim = c(0, NA), xlim = c(0, NA), expand=FALSE), 

...
p_array = as.integer(seq(20, 1.2*n, length.out=25))
...
df = df %>%
  filter(!((Method == 'lambda = 0') & (p > 0.6*n)))
...
plot_grid(fig + theme(legend.position="none"), 
        fig + 
        coord_cartesian(ylim = c(ymin, ymax)), ncol=2, align="hv", axis="tblr", labels = LETTERS[1:2])
```
...
plot_grid(fig + theme(legend.position="none"), 
        fig + coord_cartesian(ylim = c(ymin, ymax)), 
...
p_array = as.integer(seq(20, 1.2*n, length.out=25))
p_array = as.integer(seq(20, .7*n, length.out=100))
...
ymax = df %>%
  filter(((Method == 'lambda = 0') & (p < 0.5*n))) %>%
...
ymax = 1.02*max(df$rmse[df$Method == 'Pseudoinverse'])
ymin = 0.98*min(df$rmse)
...
fig = df2 %>%
  ...
  ylab("Relative reduction in rMSE (%)") +
  geom_vline(xintercept = n/2, linetype="dashed", color="grey30") + 
  coord_fixed(xlim = c(0, NA), ylim=c(0, NA), expand = FALSE) +
  scale_color_manual(name="Method",values = colMethods) + 
  geom_smooth(method='loess', span=.2, se=FALSE) 

# ymax = 1.02*max(100*df2$rel.err[df2$Method == 'Pseudoinverse'])
ymax = 100
ymin = .98*min(100*df2$rel.err[df2$Method == 'Oracle'])

...
run_simulation = function(n, epsilon, p_array, sigma_method, rho){
  ...
  df = lapply(p_array, function(p){
    ...
    data.frame(p = p,
                ...
                rMSE_baseline = normCov(cor(Y[-idx_train,])), 
  ...
  df_melt = reshape2::melt(df, id.vars=c('p', 'lambda.eb', 'rMSE_baseline'))
  df_melt
}
```
...
p_array = as.integer(seq(20, 1.2*n, length.out=12))
...
ymax = 1.02*max(100*df2$rel.err[df2$variable == 'Pseudoinverse'])
ymin = 0#.98*min(df2$value[df2$variable == 'Oracle'])
...
ymax = 1.02*max(100*df_melt$value[df_melt$variable == 'lambda = 0.01'])
ymin = 0.98*min(df_melt$value)
...
ymax = 1.02*max(df2$rel.err[df2$variable == 'Schäfer-Strimmer'])
ymin = 0.98*min(df2$rel.err)

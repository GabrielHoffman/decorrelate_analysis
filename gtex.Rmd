...
res_all %>%
  ...
  ggplot(aes(Method, rmse, fill=Method)) + 
    ...
    scale_fill_manual(name="Method", values = colMethods) +
    facet_wrap(~Tissue, ncol=5) +    
    geom_text(data=filter(res_all, rmse > cutoff), aes(Method, cutoff*.9, label=round(rmse, digits=2)), color="white")
```
...
fig1a = res_all %>%
  ...
  ggplot(aes(Method, rmse, fill=Method)) + 
    ...
    theme(aspect.ratio=1, 
      ...
      legend.position="none") +
    ylab("Root mean squared error")  +
    coord_flip(expand=FALSE, ylim=c(0, NA) ) + 
    scale_fill_manual(name="Method", values = colMethods) +    
    geom_text(data=filter(.x, rmse > cutoff), aes(Method, cutoff*.9, label=round(rmse, digits=2)), color="white") 
fig1a
```

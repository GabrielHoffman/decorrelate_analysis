...
# Plots
```{r plots, cache=FALSE, fig.width=11, fig.height=20}
res = res %>%
...
res %>% 
  group_by(Method) %>%
  summarize(elapsed = sum(elapsed) / 60) %>%
  ggplot(aes(Method, 10*elapsed, fill=Method)) +
    geom_bar(stat="identity") + 
    theme_bw()  +
    theme(aspect.ratio=1, 
      plot.title = element_text(hjust = 0.5), 
    ...
    scale_fill_manual(name="Method",values = colMethods) +
    coord_flip() +
    scale_y_log10(labels = ~.x/10, expand=c(0,0), limits=c(1, 20000), breaks  =c(10, 100, 1000, 10000)) +
...
res %>% 
  group_by(Method) %>%
  summarize(elapsed = sum(user.self) / 60) %>%
  ggplot(aes(Method, 10*elapsed, fill=Method)) +
    geom_bar(stat="identity") + 
    theme_bw()  +
    theme(aspect.ratio=1, 
    ...
    scale_fill_manual(name="Method",values = colMethods) +
    coord_flip() +
    scale_y_log10(labels = ~.x/10, expand=c(0,0), limits=c(1, 30000), breaks  =c(10, 100, 1000, 10000)) +

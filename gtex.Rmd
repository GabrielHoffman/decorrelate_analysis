...
# for each tissue
res = lapply( names(tab)[1:25], function(tissue){
  ...
  Ys = scale(log2(t(geneExpr[keep,])))[1:100,]
...
res %>%
  ggplot(aes(Method, 100*rmse / rMSE_baseline, fill=Method)) + 
    ...
    geom_text(data=filter(res, mean == cutoff), aes(Method, cutoff*.9, label=round(100*rmse / rMSE_baseline, digits=1)), color="white")

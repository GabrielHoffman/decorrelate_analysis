...
values = as.integer(sort(c(	seq(500, 20000, length.out=50), 
			seq(25000, 100000, length.out=50))))#,
		# seq(200000, 1000000, length.out=5))))


values = as.integer(sort(c(	seq(500, 20000, length.out=5))))

# values = as.integer(sort(c(	seq(500, 10000, length.out=10))))

df_values = data.frame(n = 5000, p = values, k = 20)

df_time = eval_runTime( df_values, 2000 )
...
fig = ggplot(res$df_observed, aes(p, value, color=variable)) + geom_point() + theme_classic(16) + theme(aspect.ratio=1, plot.title = element_text(hjust = 0.5), legend.position="bottom") + xlab("Number of features") + ylab("Time (seconds)") + geom_line(data=res$df_smoothed, aes(p, value, color=variable)) + scale_color_manual(name="Method", values = c("red", "blue", "green")) + ggtitle("n = 5000")

fig1 = fig + scale_y_continuous(expand=c(0,0), limits=c(0, max(df_time$naive, na.rm=TRUE))) + scale_x_continuous(limits=c(0, 20000), expand=c(0,0))
fig2 = fig + scale_y_continuous(expand=c(0,0), limits=c(0, max(df_time$naive, na.rm=TRUE))) + scale_x_continuous(expand=c(0,0))
